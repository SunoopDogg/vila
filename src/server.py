...
def load_video(video_url: str) -> str:
    ...
    temp_fpath = os.path.join(
        temp_dir, f"{uuid.uuid5(uuid.NAMESPACE_DNS, video_url)}.mp4")
...
def load_image(image_url: str) -> Image:
    if image_url.startswith("http") or image_url.startswith("https"):
        ...
        try:
            image = PILImage.open(
                BytesIO(base64.b64decode(image_base64))).convert("RGB")
...
async def lifespan(app: FastAPI):
    ...
    tokenizer, model, image_processor, context_len = load_pretrained_model(
        model_path, model_name, None, trust_remote_code=True, device_map={"": 0},)
    print(
        f"Model {model_name} loaded successfully. Context length: {context_len}")
    yield
...
async def chat_completions(request: ChatCompletionRequest):
    try:
                ...
                if isinstance(message.content, str):
                    ...
                    for content in message.content:
                        if content.type == "text":
                            ...
                            logger.info(
                                f"loading {content.frames} frames from {video}")
        ...
        input_ids = tokenizer_image_token(
            prompt_text, tokenizer, return_tensors="pt").unsqueeze(0).to(model.device)
        ...
        with torch.inference_mode():
            if request.stream:
                streamer = model.generate_content(
